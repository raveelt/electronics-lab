...
def calculate_heartrate(time_data, y_data):
    y_smooth = signal.savgol_filter(y_data, window_length=20,
                                        polyorder=3, mode="nearest") # smoothing
    df_y = np.gradient(y_smooth)                                #differentiating

    period = time_data[1] - time_data[0]                          #sample period
    sampfreq = 1/period                                        #sample frequency
    fourierTransform = np.fft.fft(df_y)                      #performing the fft
    
    #creating the range of frequencies
    y_count = len(y_data)
    values     = np.arange(int(y_count))
    timePeriod = y_count/sampfreq
    frequencies = values/timePeriod                 

    #finding the index of max value in the absolute of the fourier transform
    heartrateindex = np.where(abs(fourierTransform) == max(abs(fourierTransform)))
    heartrateindex = heartrateindex[0][0]

    heartrate = frequencies[heartrateindex] #using the index to find the freq
    heartrate *= 60   # converting to beats per minute
...
port = '/dev/tty.usbmodem1203' #For Mac?
...
# çcsv_savename = os.path.expanduser("~/Documents/Phys319/lab56p9/heartdata.csv")
csv_savename = r"/Users/raveel/Documents/Phys 319/lab56p9/heartdata.csv"
#######################################
                ...
                if len(trimdata) != ((BLOCK_DATA_LENGTH-SKIP_DATA)*2):  # make sure there are enough bytes
                    ...
                    if create_plot:
                        ...
                        if len(timeseries) < plot_last:
                            ...
                            heartrate = calculate_heartrate(timeseries,fullIRdata)
                            heartrate = np.round(heartrate)
                            #print(heartrate)
                            plt.title(str(heartrate)+ "BPM", fontsize=12)
                        else:
                           pltind = len(timeseries)-plot_last #changed this to zero plot the whole thing
                           if spo2_mode:
                               plt.plot(timeseries[pltind:],fullreddata[pltind:], label="Red")
                           plt.plot(timeseries[pltind:],fullIRdata[pltind:], label="IR")
                           heartrate = calculate_heartrate(timeseries[pltind:],fullIRdata[pltind:])
                           heartrate = np.round(heartrate)
                           #print(heartrate)
                           plt.title(str(heartrate) + "BPM", fontsize=12)
